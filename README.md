...
为了确保在编辑 README.md 时，复制的图片默认保存在 `assets` 目录并正确引用，我们已经配置了以下设置：

### VS Code 用户
- 创建了 `.vscode/settings.json` 文件，设置了图片保存路径为 `assets` 目录
- 当你在 VS Code 中粘贴图片时，图片会自动保存在 `assets` 目录，并以相对路径引用

### 其他编辑器用户
- 请参考各自编辑器的文档，设置图片保存的默认路径为 `assets` 目录
- 确保使用相对路径引用图片，格式为 `![图片描述](assets/图片文件名.png)`
